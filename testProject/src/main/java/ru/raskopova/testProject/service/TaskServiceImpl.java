...
public class TaskServiceImpl implements TaskService {
    ...
    public List<Task> getAllTasks() {
        if(taskRepository.findAll().isEmpty()){
            throw new IllegalArgumentException("There are no elements in list!");
        }
    ...
    public Task addTask(Task task) {
        ...
        return taskRepository.save(new Task()
                ...
                .setStatus(status)
                .setActionDate(date).setComment(comment));
    }

    @Override
    public Task getTaskByOrderID(int order_id) {
        if(taskRepository.findByOrderNumber(order_id) == null){
    ...
    public List<Task> getTaskByDatePeriod(LocalDate date1, LocalDate date2) {
        if(taskRepository.findAllByActionDateBetween(date1.atTime(LocalTime.MIN), date2.atTime(LocalTime.MAX)).isEmpty()){
    ...
    public List<Task> getTaskByStatus(String status) {
        if(taskRepository.findAllByStatus(status).isEmpty()){
